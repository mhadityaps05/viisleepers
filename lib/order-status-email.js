...
function renderAdminOrderItemsList(orderItems) {
  ...
  return orderItems
    .map((item) => {
      ...
      const price = escapeHtml(formatRupiah(item?.price))

      return `<li>${name} — Size: ${size} · Qty: ${quantity} · ${price}</li>`
    })
    .join("")
}
...
function renderAdminNewOrderTemplate({
  orderNumber,
  customerName,
  customerEmail,
  customerPhone,
  ...
  <html>
    ...
    <body style="margin:0;padding:24px;background:#f5f5f5;color:#111111;font-family:Arial,Helvetica,sans-serif;">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e5e5e5;border-radius:16px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:28px;">
            <h1 style="margin:0 0 12px;font-size:24px;line-height:1.3;color:#111111;">New Payment Received</h1>
            <p style="margin:0 0 22px;font-size:15px;line-height:1.7;color:#333333;">A customer payment just settled. Order details below.</p>
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="margin:0 0 22px;border:1px solid #ebebeb;border-radius:12px;overflow:hidden;">
              ...
              <tr>
                <td style="padding:16px 18px;">
                  <p style="margin:0 0 8px;font-size:14px;color:#222222;"><strong>Order Number:</strong> ${escapeHtml(orderNumber)}</p>
                  <p style="margin:0 0 8px;font-size:14px;color:#222222;"><strong>Transaction Time:</strong> ${escapeHtml(formatDate(transactionTime))}</p>
                  <p style="margin:0 0 8px;font-size:14px;color:#222222;"><strong>Payment Method:</strong> ${escapeHtml(formatPaymentMethod(paymentMethod))}</p>
                  <p style="margin:0 0 10px;font-size:14px;color:#222222;"><strong>Total Payment:</strong> ${escapeHtml(formatRupiah(total))}</p>
                  <p style="margin:0 0 8px;font-size:14px;color:#222222;"><strong>Items:</strong></p>
                  <ul style="margin:0;padding-left:18px;font-size:14px;line-height:1.7;color:#222222;">
                    ${renderAdminOrderItemsList(orderItems)}
                  </ul>
                </td>
              </tr>
            </table>
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="margin:0;border:1px solid #ebebeb;border-radius:12px;overflow:hidden;">
              <tr>
                <td style="padding:16px 18px;background:#fafafa;border-bottom:1px solid #ebebeb;font-size:13px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;color:#666666;">Customer</td>
              </tr>
              <tr>
                <td style="padding:16px 18px;">
                  <p style="margin:0 0 8px;font-size:14px;color:#222222;"><strong>Name:</strong> ${escapeHtml(customerName)}</p>
                  <p style="margin:0 0 8px;font-size:14px;color:#222222;"><strong>Email:</strong> ${escapeHtml(customerEmail)}</p>
                  <p style="margin:0;font-size:14px;color:#222222;"><strong>Phone:</strong> ${escapeHtml(customerPhone)}</p>
                </td>
              </tr>
            </table>
          </td>
...
export async function sendAdminPaymentNotificationEmail({
  ...
  await resend.emails.send({
    ...
    subject: `New Payment Received – ${orderNumber}`,
    html: renderAdminNewOrderTemplate({
      orderNumber,
      customerName,
      customerEmail,
      customerPhone,
...
export async function sendPaymentSuccessEmail({
  ...
  const firstProductName = getFirstProductName(productNames)
  ...
  await sendStatusEmail({
    ...
    createdAt,
    subject: `Payment Confirmed – ${firstProductName}`,
    heading: "Payment Successful",
    intro:
      "Thank you for your purchase! We've successfully received your payment and your order has been confirmed.",
    extraContent:
      '<p style="margin:0;font-size:14px;line-height:1.7;color:#333333;">Our team will begin preparing your order shortly.<br/>We\'ll keep you updated by email as your order progresses.</p>',
  })
